...
func ta_revisions(baseUrl, repo string, rversions []string, results [][]string) [][]string {
	for rv := range rversions {
		// https://qa.door43.org/api/v1/repos/unfoldingword/en_ta/git/trees/v24?recursive=true
		...
		for key, val := range jsonMap {
			if key == "tree" {
				...
				for _, tval := range v {
					...
					//fmt.Printf("%v,%v,%v,%v,%v\n", repo, rversions[rv], nodeMap["path"], nodeMap["type"], nodeMap["sha"])
					path := fmt.Sprintf("%v", nodeMap["path"])
					ntype := fmt.Sprintf("%v", nodeMap["type"])
					if ntype == "tree" && strings.HasPrefix(path, "translate/") {
						// example: translate/figs-hyperbole
						snippets := strings.Split(path, "/")
						article := snippets[1]
						row = append(row, repo, rversions[rv], article, fmt.Sprintf("%v", nodeMap["sha"]))
						//fmt.Printf("Appending row: %v\n", row)
